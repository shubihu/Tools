...
class Notarget(Template):
	def __init__(self, path):
		...
		self.project_name = self.projectinfo.loc['项目名称'][1]
		self.school = self.projectinfo.loc['委托单位'][1]
		self.project_num = self.projectinfo.loc['项目编号'][1]
		self.groupvs = self.projectinfo.loc['比较组'][1]
	...
	def table_data(self, tables):
		...
		self.insert_table(pd.read_csv('sample.txt', sep='\t'), tables[1], size=12, family_en='Times New Roman')
	...
	def text_png_data(self, paragraphs):
		for i, p in enumerate(paragraphs):
			...
			if '[pos-tic]' in p.text:
				self.insert_png(p, '[pos-tic]', 'pos-tic.png')
			if '[neg-tic]' in p.text:
				self.insert_png(p, '[neg-tic]', 'neg-tic.png')
			...
			if '[pos_cluster]' in p.text:
				self.insert_png(p, '[pos_cluster]', os.path.join('报告及附件', '附件2 Result', '04. Bioinformatics Analysis', 'Hierarchical Clustering Analysis', self.groupvs, 'pos-cluster.png'))
			...
			if '[neg_cluster]' in p.text:
				self.insert_png(p, '[neg_cluster]', os.path.join('报告及附件', '附件2 Result', '04. Bioinformatics Analysis', 'Hierarchical Clustering Analysis', self.groupvs, 'neg-cluster.png'))
			...
			if '[kegg_heatmap]' in p.text:
				self.insert_png(p, '[kegg_heatmap]', os.path.join('报告及附件', '附件2 Result', '04. Bioinformatics Analysis', 'KEGG Analysis', self.groupvs, 'KEGG Pathway_Metabolites Heatmap', f'{self.keggId}_heatmap.png'))
	...
	def save(self, document):
		document.save('非靶代谢报告.docx')
		print(f'报告生成完成，报告路径为：{os.path.join(os.getcwd(), "非靶代谢报告.docx")}')
